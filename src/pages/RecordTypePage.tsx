  ...
  return (
    <div>
      <StudyTemplate
        ...
        learned={`
        Record<키타입, 값타입> 객체의 key와 value의 타입을 한 번에 정의할 수 있는 유틸리티 타입
        이 객체는 이런 키들을 가지고 있고 값은 전부 이 타입이다
        레코드를 사용할 때 키는 무엇이 될 수 있을까 / 값은 항상 어떤 형태인가 고민
        필수기재 폼이랑 비슷한 느낌이다 그렇게 이해하기로 했지만 더 고민해봐야 겠다 
        항목이 정해져 있고 빠트리면 안되고 멋대로 항목을 추가하면 안되고 항목마다 작성형식 정해져 있다 ! 
      ...
      />
        ...
        title="문제 1"
        question={`
            전자상거래 플랫폼에서 지역 코드에 따른 배송비를 계산하는 로직을 작성하세요.
...
// console.log(calculateShippingCost("AUSTRALIA", shippingCosts)); // 에러 발생
      ...
      />
        ...
        title="문제 2"
        question={`
            학생들의 점수를 기록하고 평균 점수를 계산하는 문제입니다.
...
console.log(calculateAverageScore(scores)); // 85
      ...
      />
        ...
        title="문제 3"
        question={`
            쇼핑몰에서 각 제품의 이름과 가격을 매핑하고, 특정 제품의 가격을 업데이트하는 기능을 구현하세요.
