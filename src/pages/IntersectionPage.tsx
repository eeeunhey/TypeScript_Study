...
const IntersectionPage = () => {
  return (
    <div>
      <StudyTemplate
        title="6강- 교차 타입이란 무엇인가 "
        learned={`
          교차 타입(&) 은 A이면서 동시에 B인 하나의 타입을 만드는 것이다.
          A & B는 A 조건도 만족하고 B 조건도 만족해야만 사용할 수 있다.
          그래서 교차 타입은 여러 속성을 하나로 합친 객체이다
          이것도 되고 저것도 되는 값 이 아니고 이것도 반드시 되고 저것도 반드시 돼야 하는 값이다.
          내가 이해할 때는 그렇다
          교차 타입은 타입을 늘리거 같이 보이지만은 이것도 되야하고 저것도 되야하니 조건이 더 붙는다
          `}
        reason={``}
      />
      <CodeBlock 
      title={`문제1`} 
      question={`
        상품(Product)과 할인(Discount) 정보를 병합하여 새로운 타입을 정의하고, 할인 적용 후의 가격을 계산하는 함수를 작성하세요.
        1. Product 타입:
            id: 숫자
            name: 문자열
            price: 숫자
        ...
        3. Product & Discount 타입을 기반으로 할인된 가격을 반환하는 함수를 작성하세요:
...
function calculateDiscountedPrice(item: Product & Discount ): number {
      ...
      // const discountRate = item.discountPercentage / 100
      // const discountAmount = originalPrice * discountRate;
      // return originalPrice - discountAmount;

    return item.price - (item.price * item.discountPercentage) / 100;
    // item.price (상품 원래가격) - (원래가격 * 할인율) / 100
}
...
console.log(calculateDiscountedPrice(discountedProduct)); // 800
    ...
    <CodeBlock title={`문제 2`} 
      question={`
 아래의 조건에 따라 복합 데이터를 처리하는 타입을 정의하고, 관련된 함수를 작성하세요.
1. ContactInfo 타입:
    phone: 문자열
    address: 문자열
    OrderInfo 타입:
...
console.log(printOrderSummary(orderDetails)); // "Order 2023 (Phone: 123-456-7890)"
      ...
      3. Profile & Activity 타입을 기반으로 다음 요구사항을 만족하는 코드를 작성하세요:
...
function getUserSummary(user: Profile & Activity): string {
  ...
  //toISOString()은 날짜를 표준 문자열 형식으로 바꿔준다
}
